...
| `⌘6` Agents, `⌘1`–`⌘7` (#37) | Agents takes `⌘6` after Stats; Pricing moves to `⌘7` | Upstream's tab order puts Agents after Stats. Same mechanism as Hourly: one row in `NAV`, no change to `keys.ts`. |
| `⌘5` Minutely, when enabled (#38) | Minutely takes `⌘5` after Hourly and everything after it moves down one; hidden, the numbering is what it was | Upstream's tab order. The shell filters `NAV` once and the sidebar, the `⌘`-digits and the sheet all read the filtered list, so they cannot disagree about a hidden View. |
| `⌘2` Usage, `⌘1`–`⌘8` (#39) | Usage takes `⌘2` after Overview and everything after it moves down one | Upstream's `Tab` enum puts Usage second. With Minutely enabled the sidebar is nine destinations, which is `⌘9` and the last digit the resolver answers: the next View needs a decision here, not a row in `NAV`. |
| `⌘,` Settings (#38) | Opens the Settings sheet | Every macOS app's Settings key, so it needs no discovery. A modifier, so it works from a text field. Matched on `event.key`, like `?`. The sidebar also has a Settings button. |
...
| `R` toggle auto-refresh, `+` / `-` interval | Re-decided when #38 built interval refresh, and still rejected. Turning it on schedules a Scan every few minutes for as long as the window is open, and a key press is the wrong way to start that: a stray Shift on `R` would do it with nothing on screen to say so. It lives on the Settings sheet (`⌘,`), where the checkbox shows its state and the sentence under it says what a tick costs. Shift+R stays a Refresh, so the sheet's `R` row stays true. `+`/`-` fall with it: the interval is a number field beside the checkbox. |
