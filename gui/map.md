...
## Amendment, 2026-09-06: P1 is built

The map's `## Notes` say "This map plans; it does not build", and `## Out of scope`
lists "Implementing P1-P3". **That is no longer what the tree contains**, and the
split is recorded here rather than quietly rewritten, because whether to change
the destination is not a decision a session should take on its own.

What happened: closing every remaining ticket would still have left a navigable
shell with placeholder text, because #10 and #12 are decide-tickets too. Running
the app made that concrete, so P1 was built.

Now in the tree, beyond the map's plan:

- **The five commands** ticket 09 designed (`scan`, `model_report`, `graph_report`,
  `clients`, `settings`) plus `unpriced`, over a Snapshot held in `tauri::State`.
  The fork gained three `pub` keywords and nothing else, exactly as 09 specified.
- **Overview, Models, Daily and Stats** against real data, in ticket 07's register.
- **A Pricing view** editing upstream's `~/.config/tokscale/custom-pricing.json`,
  so models no source can price get a manual rate. Not in any ticket; it came from
  a direct request.

Two departures from ticket 09, both found by building it:

- The boundary is hand-declared in `src-tauri/src/dto.rs` rather than generated by
  `tauri-specta`. 09 chose derives-in-fork because DTOs meant ~150 duplicated
  fields; P1 actually reads 15. **`specta` is therefore not enabled**, and the
  Rust >= 1.92 floor 09 called load-bearing is currently not load-bearing at all
  — which changes the answer to the open ticket "Pin the Rust toolchain floor".
- `scan` returns a held Snapshot unless forced, because a webview reload drops the
  frontend's query cache but not the corpus, and was costing a full rescan.

**Correction to a measured number.** Ticket 14 records a cold scan at 21-40 s.
On this machine it is now **~2 minutes** at 14,958 messages. Ticket 07's loading
states were designed against the old figure, and the still-open "Design the cold
first-run experience" depends on it, so the number matters.

**Still unverified:** the Pricing save round-trip through the UI. The window's
webview stops painting when it is not focused and this environment cannot focus
or click it, so the write path is pinned by tests against core's own reader
instead. One human click would settle it.
